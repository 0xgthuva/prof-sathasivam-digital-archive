...
function academix_fix_team_slug($args, $post_type) {
    if ($post_type === 'team') {
        $args['rewrite'] = array(
            'slug' => 'about',
            'with_front' => false
...
add_filter('register_post_type_args', 'academix_fix_team_slug', 10, 2);

function academix_fix_book_slug($args, $post_type) {
    if ($post_type === 'book') {
        $args['rewrite'] = array(
            'slug' => 'books',
            'with_front' => false
...
add_filter('register_post_type_args', 'academix_fix_book_slug', 10, 2);


function hide_team_post_type_from_admin() {
    remove_menu_page('edit.php?post_type=journal_article');
}
add_action('admin_menu', 'hide_team_post_type_from_admin', 999);